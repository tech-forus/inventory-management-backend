    ...
    static async getItemHistory(skuId, companyId, filters = {}) {
        ...
        SELECT * FROM (
          ...
          SELECT
            ...
            0 as rejected,
            0 as short,
            COALESCE(c.customer_name, d.name, oi.destination_name) as source_destination,
            oi.challan_number,
            oi.challan_date,
            oii.updated_at,
            oii.created_at,
          ...
          LEFT JOIN destinations d ON oi.destination_id = d.id AND oi.destination_type = 'destination'
          WHERE s.sku_id = $1 
            AND oi.company_id = $2
