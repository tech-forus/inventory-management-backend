...
class VendorModel {
  ...
  static async create(vendorData, companyId) {
    const result = await pool.query(
      `INSERT INTO vendors (
        company_id, name, contact_person, designation, email, phone, gst_number,
  ...
  static async update(id, vendorData, companyId) {
    const result = await pool.query(
      `UPDATE vendors SET
        ...
        phone = $5, gst_number = $6, address = $7, city = $8,
        state = $9, pin = $10, is_active = $11, updated_at = CURRENT_TIMESTAMP
