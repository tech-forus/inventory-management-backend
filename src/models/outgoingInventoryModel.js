  ...
  static async create(inventoryData, items, companyId) {
    ...
    try {
      ...
      const totalValue = items.reduce((sum, item) => {
        return sum + (item.totalValue || (item.outgoingQuantity * item.unitPrice));
      }, 0);
      ...
      for (const item of items) {
        const itemValue = item.totalValue || (item.outgoingQuantity * item.unitPrice);
        const outgoingQty = item.outgoingQuantity || 0;
