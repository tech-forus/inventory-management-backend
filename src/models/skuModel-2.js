...
class SKUModel {
  ...
  static async getAll(filters, companyId) {
    ...
    // Add filters
    if (filters.search) {
      query += ` AND (s.sku_id ILIKE $${paramIndex} OR s.item_name ILIKE $${paramIndex} OR s.model ILIKE $${paramIndex})`;
  ...
  static async update(id, skuData, companyId = null) {
    let query = `
      UPDATE skus SET
        ...
        weight = $23, length = $24, width = $25, height = $26,
        min_stock_level = $27, reorder_point = $28, default_storage_location = $29,
        status = $30, is_active = $31, updated_at = CURRENT_TIMESTAMP
      WHERE id = $32
    `;
    ...
    // Add company ID filter if provided
    if (companyId) {
      query = query.replace('WHERE id = $32', `WHERE id = $32 AND company_id = $33`);
      params.push(companyId.toUpperCase());
    }
