...
const transformIncomingHistory = (record) => {
  ...
  return {
    ...
    totalValue: parseFloat(record.total_value || 0),
    total_value_excl_gst: parseFloat(record.total_value_excl_gst || 0),
    totalValueExclGst: parseFloat(record.total_value_excl_gst || 0),
    total_value_incl_gst: parseFloat(record.total_value_incl_gst || 0),
    totalValueInclGst: parseFloat(record.total_value_incl_gst || 0),
    gst_amount: parseFloat(record.gst_amount || 0),
    gstAmount: parseFloat(record.gst_amount || 0),
...
/**
 * Get all incoming inventory items with warranty and serial numbers
 */
const getAllWarrantySerialItems = async (req, res, next) => {
  try {
    ...
    const items = await IncomingInventoryModel.getAllWarrantySerialItems(companyId, filters);
